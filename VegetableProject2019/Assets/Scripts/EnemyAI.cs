...
//      m_rigidbody = GetComponent<Rigidbody2D>();
    ...
    void MoveHorizontal(GameObject target)
    {
        ...
        if(Mathf.Abs(delta) < 6f && Mathf.Abs(delta) > 3.3f)
        {
            print("Атакую ");
            m_attack.input(true);
        }
        ...
        Vector2 horiz_axis = new Vector2(target.transform.position.x, transform.position.y);
        transform.position = Vector2.MoveTowards(transform.position, horiz_axis, speed * Time.deltaTime);
    ...
    GameObject FindTarget()
    {
        ...
        if (hitTop.collider != null && hitBottom.collider.gameObject != null && hitBottom.collider.gameObject.GetComponent<Player>() != null)
        {
